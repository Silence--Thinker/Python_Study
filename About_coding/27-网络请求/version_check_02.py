...
def request_get_versions(bundle_name, size, test=True):
    ...
    pass
    # print('======= {} {} ======='.format(bundle_name, '测试' if test else '正式'))
...
def cooking_version_info(bundle_dict, version_prefix='',queue=None):
    ...
    if not queue.empty():
        ...
        print('========================== {} =========================={}'.format(bundle_name, threading.currentThread()))
        # get versions
...
def main(version_prefix='7.32'):
    ...
    for bundle in bundle_list:
        ...
        # thread = customThread(target=cooking_version_info, args=(bundle_dict, version_prefix, workQueue), name=bundle)
        thread.start()
